...
class FinancialPipelineTestSuite:
    ...
    def _test_macrotrends_data_pipeline(self):
        ...
        print(
            f"   📅 Date range: {self.df.index.min().strftime('%Y-%m-%d')} to {self.df.index.max().strftime('%Y-%m-%d')}"
        )
        print(f"   🔢 Index type: {type(self.df.index).__name__}")

        # Verify essential financial columns
        essential_columns = ["FCF-LTM", "Shares-Outstanding", "Market-Cap", "Revenue", "Operating-Income", "Net-Income"]
    ...
    def _test_wacc_integration_validation(self):
        ...
        wacc_components = ["Risk-Free-Rate", "beta", "Equity-Risk-Premium", "Cost-of-Equity", "Cost-of-Debt", "WACC"]
    ...
    def _test_dcf_model_integration(self):
        ...
        pv_fcf_fraction, dcf_val, stock_price = self.dcf_model.calculate_dcf(
            self.df, discount_rate_scale=1.0, terminal_growth=0.025, time_horizon=10
        )
    ...
    def _test_market_analysis_diagnostics(self):
        ...
        print(
            f"      Date Consistency: {'✓' if date_consistency else '✗'} {'Monotonic' if date_consistency else 'Non-monotonic'}"
        )
