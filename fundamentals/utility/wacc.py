...
def compute_rolling_beta(
    ticker_symbol, risk_free_quarterly, start_date="2010-01-01", end_date="2025-06-01", window_months=24, verbose=False
):
    """
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if len(cells) > 4:
                    ...
                    # Look for header row
                    if any("Year" in text for text in cell_texts) and any(
                        "ERP" in text or "Premium" in text for text in cell_texts
                    ):
                        header_found = True
