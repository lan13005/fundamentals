...
This module provides a DCF model implementation with Monte Carlo simulation
...
class StandardDCFModel:
    """
    DCF model implementation with Monte Carlo simulation capabilities.

    This class provides comprehensive DCF analysis with:
    - Prior distribution specification and parameter uncertainty quantification
    - Monte Carlo sampling with correlation support via Gaussian copula
    - Sliding window analysis across time periods
    - Constraint enforcement for valid parameter combinations
    - Diagnostic plotting capabilities

    Architecture Overview:
    ---------------------
    The class follows a clean hierarchical design with functional parameter passing:
    
    1. `calculate_dcf()` - Core DCF calculation with optional center_idx parameter
    2. `simulate()` - Wraps calculate_dcf with Monte Carlo parameter sampling, passes center_idx through
    3. `sliding_dcf_analysis()` - Wraps simulate across multiple time periods using center_idx parameter
    
    Parameter Flow:
    ---------------
    - `center_idx` parameter flows cleanly through: sliding_dcf_analysis → simulate → calculate_dcf
    - No global state changes - all data subsetting happens functionally via center_idx
    - Each method can be called independently with appropriate parameters

    Core Methods:
    -------------
    - `configure_priors()` - Set up parameter distributions and correlations
    - `calculate_dcf()` - Core DCF calculation (can accept center_idx)
    - `simulate()` - Monte Carlo simulation using parameter sampling
    - `sliding_dcf_analysis()` - Time-series sliding window analysis
    - `plot_corner_diagnostics()` - Parameter and result visualization
    - `plot_terms_diagnostics()` - FCF vs Terminal value visualization

    Required Parameters for DCF calculation:
    ---------------------------------------
    - discount_rate_scale: Scaling factor for WACC (float)
    ...
    - forecast_horizon: Number of years to forecast forward (int)

    Usage Examples:
    ---------------
    # Basic setup
    model = StandardDCFModel(df)
    model.configure_priors(distributions, correlation_matrix, cagr_lookback=10.0)
    
    # Single DCF calculation for latest date
    pv_fcf_fraction, dcf_value, stock_price = model.calculate_dcf(
        discount_rate_scale=1.0, terminal_growth=0.025, forecast_horizon=10
    )
    
    # DCF calculation for a specific historical date
    pv_fcf_fraction, dcf_value, stock_price = model.calculate_dcf(
        discount_rate_scale=1.0, terminal_growth=0.025, forecast_horizon=10, center_idx=50
    )
    
    # Monte Carlo simulation (latest date) - all parameters sampled
    pv_fcf_fractions, dcf_values, stock_prices, parameter_samples = model.simulate(n_samples=1000)
    
    # Monte Carlo simulation with parameter overrides
    pv_fcf_fractions, dcf_values, stock_prices, parameter_samples = model.simulate(
        n_samples=1000, discount_rate_scale=1.2, terminal_growth=0.03
    )
    
    # Monte Carlo simulation for specific historical date
    pv_fcf_fractions, dcf_values, stock_prices, parameter_samples = model.simulate(
        n_samples=1000, center_idx=50
    )
    
    # Sliding window analysis - all parameters sampled
    results_df = model.sliding_dcf_analysis(cagr_lookback=5.0, forecast_horizon=10, n_samples=1000)
    
    # Sliding window analysis with parameter overrides
    results_df = model.sliding_dcf_analysis(
        cagr_lookback=5.0, forecast_horizon=10, n_samples=1000,
        discount_rate_scale=1.1, terminal_growth=0.025  # Fixed across all dates
    )
    
    # Diagnostic plots
    fig, axes = model.plot_corner_diagnostics(stock_prices, parameter_samples)
    fig, ax = model.plot_terms_diagnostics(stock_prices, pv_fcf_fractions)
    """
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        """
        self.df = df
        self._original_df = df.copy()  # Store original for sliding analysis
        self.distributions = {}
        self.parameter_names = []
        self._correlation_matrix = None


    def configure_priors(self, distributions: Dict[str, Any], correlation_matrix: Optional[np.ndarray] = None, cagr_lookback: float = 10.0) -> None:
        """
        ...
        cagr_lookback : float, optional
            Number of years to look back for empirical CAGR calculation when growth_rate=None (default=10.0)
        ...
        model.configure_priors(distributions, correlation, cagr_lookback=10.0)
        """
        ...
        self._original_cagr_lookback = cagr_lookback

        # Required parameter distributions
        required_params = ["discount_rate_scale", "terminal_growth", "forecast_horizon"]
        ...
        for param_name, dist in distributions.items():
            if param_name == "growth_rate":
                if dist is None:
                    ...
                    #  Hard to ensure positivity with CAGR formula
                    annual_fcf = self.df["FCF-LTM"]
                    mean_growth = annual_fcf.iloc[-cagr_lookback*4:-1].pct_change().mean()
                    std_growth  = annual_fcf.iloc[-cagr_lookback*4:-1].pct_change().std() * 0.1
                    processed_distributions[param_name] = stats.norm(loc=mean_growth, scale=std_growth)
                    param_types[param_name] = "empirical_with_correlation"
                    logger.debug(f"growth_rate: Using empirical mean growth rate {mean_growth:.2%} with std {std_growth:.2%} (lookback={cagr_lookback} years)")
                elif isinstance(dist, int | float):
                    # Convert numeric to delta function
                    scale = abs(dist) * 1e-6 if not np.isclose(dist, 0) else 1e-6
        ...
        all_param_names = [*required_params, "growth_rate"]
        ...
        for param_name, dist in self.distributions.items():
            ...
            logger.debug(f"{param_name}: Mean={samples.mean():.2f}, Std={samples.std():.2f}")
    ...
    def calculate_dcf(
        ...
        growth_rate : float
            Growth rate to use for FCF projections (default=0.05)
        ...
        center_idx : int, optional
            If provided, temporarily subset data to [:center_idx+1] for calculation

        Returns:
        --------
        tuple : (pv_fcf_fraction, dcf_value, stock_price)
        """
        # Handle center_idx if provided
        if center_idx is not None:
            self._set_center(center_idx)
        ...
        try:
            ...
            projected = [last_fcf * (1 + g) ** t for t in range(1, int(forecast_horizon) + 1)]
            ...
            pv_term = terminal / (1 + discount_rate) ** int(forecast_horizon)
    ...
    def simulate(
        ...
        center_idx : int, optional
            If provided, temporarily subset data to [:center_idx+1] for all DCF calculations
        ...
        if show_progress:
            with Progress(
                ...
                results = self._run_simulation(n_samples, random_state, progress, center_idx)
        else:
            results = self._run_simulation(n_samples, random_state, None, center_idx)
    ...
    def sliding_dcf_analysis(
        ...
        if valid_end_idx > valid_start_idx:
            ...
            logger.warning(f"No valid center dates available. Need at least {quarters_needed} quarters of data.")
        ...
        with Progress(
            ...
            for center_idx in range(valid_start_idx, valid_end_idx):
                ...
                try:
                    # Run Monte Carlo simulation for this center date using center_idx parameter
    ...
    def _run_simulation(
        ...
        for i in range(n_samples):
            ...
            pv_fcf_fraction, dcf_val, stock_price = self.calculate_dcf(**params, center_idx=center_idx)
    ...
    def _set_center(self, center_idx: int) -> None:
        """
        Internal method to set the center date for sliding window analysis.
        ...
        if hasattr(self, '_original_distributions') and hasattr(self, '_original_correlation_matrix') and hasattr(self, '_original_cagr_lookback'):
            self.configure_priors(self._original_distributions, self._original_correlation_matrix, self._original_cagr_lookback)
        else:
