...
def print_rich_help():
    ...
    table.add_row(
        "diagnose-cursor-mcp",
        "Diagnose Cursor MCP logs for errors and successes. Optionally specify a range of logs to analyze.",
    )
    ...
    table.add_row(
        ...
        "Build a Sector-Industry summary from Yahoo Finance classifications. Outputs CSV files with summary and full company info.",
    )
    console.print(table)
...
def main():
    parser = argparse.ArgumentParser(description=argparse.SUPPRESS, add_help=False)
    subparsers = parser.add_subparsers(title="subcommands", dest="command", required=True)

    # diagnose-cursor-mcp
    parser_dcm = subparsers.add_parser("diagnose-cursor-mcp", help=argparse.SUPPRESS)
    ...
    # swap-mdc
    parser_swap = subparsers.add_parser("swap-mdc", help=argparse.SUPPRESS)
    parser_swap.set_defaults(func=run_swap_mdc)

    # spread-sim
    parser_spread = subparsers.add_parser("spread-sim", help=argparse.SUPPRESS)
    parser_spread.set_defaults(func=run_spread_sim)

    # sector-industry-list
    parser_sector = subparsers.add_parser("sector-industry-list", help=argparse.SUPPRESS)
